    ...
    <script>
        ...
        function renderTimetable(timetables) {
            ...
            for (const [classe, emploiDuTemps] of Object.entries(timetables.timetables)) {
                ...
                // Ajouter une légende pour les types de cours
                const legend = document.createElement('div');
                legend.className = 'flex flex-wrap gap-4 mt-4 justify-center text-sm';

                // Créer la légende pour chaque type
                const legendItems = [{
                        type: 'default',
                        label: 'Cours'
                    },
                ...
                for (const item of legendItems) {
                    const colors = moduleColors[item.type];
                    const legendItem = document.createElement('div');
                    legendItem.className = 'flex items-center';
                    legendItem.innerHTML = `
                <div class="w-4 h-4 mr-1 rounded" style="background-color: ${colors.border}"></div>
                <span>${item.label}</span>
            `;
                    legend.appendChild(legendItem);
                }

                classeSection.appendChild(legend);
            }
        }
    </script>
@endsection
